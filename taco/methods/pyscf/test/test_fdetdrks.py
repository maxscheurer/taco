...
#
# Author: Cristina E. Gonzalez-Espinoza <crisbeth46@gmail.com>
# Based on file of Qiming Sun <osirpt.sun@gmail.com>
#
...
mf_lda.scf()
mygrids = mf_lda.grids
ao_mol0 = eval_ao(mol, mygrids.coords, deriv=0)
dm = mf_lda.make_rdm1()
rho = eval_rho(mol, ao_mol0, dm, xctype='LDA')
exc, vxc, fxc, kxc = libxc.eval_xc(mf_lda.xc, rho, 0, 0, 2)
...
def tearDownModule():
    global mol, mf, mf_lda3, mf_lda
    mol.stdout.close()
    del mol, mf, mf_lda3, mf_lda

class KnownValues(unittest.TestCase):
    def test_nohbrid_lda(self):
        td = rks.FDETDDFTNoHybrid(mf_lda3, vxc, fxc)
        es = td.kernel(nstates=5)[0] * 27.2114
        self.assertAlmostEqual(lib.finger(es), -41.059050077236151, 6)

    def test_tddft_lda(self):
        td = rks.FDETDDFT(mf_lda3, vxc, fxc)
        es = td.kernel(nstates=5)[0] * 27.2114
        self.assertAlmostEqual(lib.finger(es), -41.059050077236151, 6)

    def test_tda_lda(self):
        td = rks.FDETDA(mf_lda, vxc, fxc)
        es = td.kernel(nstates=5)[0] * 27.2114
 ...
 #  def test_dRPA(self):
 #      td = rks.FDEdRPA(mf_lda, vxc, fxc)
 #      td._scf.xc = ''
 #      es = td.kernel(nstates=3)[0]
 #      self.assertAlmostEqual(lib.finger(es), 0.32727702719009616, 6)

 #  def test_dTDA(self):
 #      td = rks.FDEdTDA(mf_lda, vxc, fxc)
 #      td._scf.xc = ''
 #      es = td.kernel(nstates=3)[0]
